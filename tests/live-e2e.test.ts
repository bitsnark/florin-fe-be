/**
 * Live end-to-end test for the LTC → Sepolia → L2 (Liteforge) flow.
 *
 * Flow:
 *   1. User creates a Liteforge reservation on Sepolia (ownerAddress = LiteforgeDepositor)
 *   2. User sends LTC to the assigned P2TR address
 *   3. florin-mm detects the LTC payment and settles on Sepolia (state=4)
 *   4. florin-mm calls LiteforgeDepositor.bridge() to bridge tokens to Liteforge L2
 *   5. florin-fe-be indexes the Bridged event → reservation shows liteforgeTxhash
 *
 * Prerequisites:
 *   1. Set TEST_EVM_PRIVATE_KEY in .env (Sepolia wallet with ETH for gas; must NOT be the MM relayer)
 ...
 *   4. Set LITEFORGE_DEPOSITOR_ADDRESS in .env (deployed LiteforgeDepositor on Sepolia)
 *   5. Set FLORIN_API_URL in .env (default: http://35.239.92.14)
 *   6. Ensure the MM has an active position with available P2TR addresses
 *
 * Run with:
 *   npx jest tests/live-e2e.test.ts --forceExit --verbose
 *
 * After the test, update TEST_LTC_UTXO_* to the change output printed by the test.
 */
...
const LITEFORGE_DEPOSITOR_ADDR = process.env.LITEFORGE_DEPOSITOR_ADDRESS || '';
...
const PHASE_TIMEOUT = 60 * 60 * 1000; // 60 min per phase (LTC testnet blocks can be slow)
const TOTAL_TIMEOUT = 120 * 60 * 1000;
...
describe('Live E2E: LTC → Sepolia → L2 (Liteforge) flow', () => {

    it('user sends LTC, florin-mm settles on Sepolia and bridges tokens to Liteforge L2', async () => {

        // ── 0. Guard rails ─────────────────────────────────────────────────
        if (!TEST_EVM_PRIVATE_KEY)        throw new Error('Set TEST_EVM_PRIVATE_KEY in .env (must NOT be the MM relayer key)');
        ...
        if (!LITEFORGE_DEPOSITOR_ADDR)    throw new Error('Set LITEFORGE_DEPOSITOR_ADDRESS in .env');
        ...
        progress(`LiteforgeDepositor:      ${LITEFORGE_DEPOSITOR_ADDR}`);

        const txout = await ltcRpc('gettxout', TEST_LTC_UTXO_TXID, TEST_LTC_UTXO_VOUT, true) as Record<string, unknown> | null;
        if (!txout) throw new Error(`UTXO ${TEST_LTC_UTXO_TXID}:${TEST_LTC_UTXO_VOUT} is already spent or not found. Fund ${ltcAddress} and update TEST_LTC_UTXO_* in .env`);
        ...
        if (contractPosition.status !== 1n && contractPosition.status !== 1) {
            throw new Error(`Position status=${contractPosition.status} is not Active (1).`);
        }
        if (availableCount === 0) {
            throw new Error('Position has 0 available P2TR addresses.');
        }

        // ── 3. Create Liteforge reservation on Sepolia ────────────────────
        // ownerAddress = LiteforgeDepositor so florin-mm recognises this as a
        // Liteforge reservation and will bridge to L2 after settlement.
        progress('\n=== STEP 3: Creating Liteforge reservation on Sepolia ===');
        progress(`ownerAddress = LiteforgeDepositor (${LITEFORGE_DEPOSITOR_ADDR})`);

        const exchangeWithSigner = new ethers.Contract(CONTRACT_ADDRESS, exchangeAbi, evmWallet);
        progress(`Calling reservePosition(${positionId}, ${LITEFORGE_DEPOSITOR_ADDR}, ${TOKEN_AMOUNT})…`);

        const tx = await exchangeWithSigner.reservePosition(positionId, LITEFORGE_DEPOSITOR_ADDR, TOKEN_AMOUNT);
        ...
        progress(`LTC destination scriptPubKey: ${p2trScriptHex}`);
        ...
        await pollUntil(
            ...
            r => r !== null,
            (r, attempt) => {
                if (r) progress(`  [attempt ${attempt}] Indexed! state=${r.state}`);
                else   progress(`  [attempt ${attempt}] Not yet indexed (404)…`);
            },
            PHASE_TIMEOUT,
        );

        progress('Reservation indexed.');

        // ── 5. Send LTC ───────────────────────────────────────────────────
        progress('\n=== STEP 5: Sending LTC ===');
        progress(`  From:   ${ltcAddress}`);
        progress(`  To:     scriptPubKey ${p2trScriptHex}`);
        progress(`  Amount: ${PAYMENT_SATS} sats`);
        ...
        progress(`Change UTXO (update .env after test): TEST_LTC_UTXO_TXID=${ltcTxid} TEST_LTC_UTXO_VOUT=1 TEST_LTC_UTXO_AMOUNT_SATS=${TEST_LTC_UTXO_AMOUNT_SATS - PAYMENT_SATS - FEE_SATS}`);
        ...
        await pollUntil(
            ...
            (r, attempt) => {
                if (!r) {
                    ...
                    progress(`  [attempt ${attempt}] LTC tx indexed! originTxhash=${r.originTxhash}`);
                } else {
                    progress(`  [attempt ${attempt}] LTC tx not yet indexed. Current LTC block: ${r.blockCount ?? '?'}. state=${r.state}`);
                }
            },
            PHASE_TIMEOUT,
        );

        // ── 7. Wait for florin-mm to settle on Sepolia ────────────────────
        progress('\n=== STEP 7: Waiting for florin-mm to settle reservation on Sepolia ===');
        ...
        const settled = await pollUntil(
            ...
            (r, attempt) => {
                const rec = r as Record<string, unknown>;
                progress(`  [attempt ${attempt}] state=${rec?.state} targetTxhash=${rec?.targetTxhash ?? 'none'}`);
            },
            PHASE_TIMEOUT,
        );

        progress(`Settlement confirmed: targetTxhash=${settled.targetTxhash}`);

        // ── 8. Wait for florin-mm to bridge to Liteforge L2 ──────────────
        progress('\n=== STEP 8: Waiting for florin-mm to bridge tokens to Liteforge L2 ===');
        progress('(florin-mm calls LiteforgeDepositor.bridge() after settlement; florin-fe-be indexes the Bridged event)');

        const bridged = await pollUntil(
            'bridge to L2',
            () => getReservation(reservationId!),
            r => r !== null && typeof (r as Record<string, unknown>).liteforgeTxhash === 'string',
            (r, attempt) => {
                const rec = r as Record<string, unknown>;
                progress(`  [attempt ${attempt}] liteforgeTxhash=${rec?.liteforgeTxhash ?? 'none'}`);
            },
            PHASE_TIMEOUT,
        );

        const rec = bridged as Record<string, unknown>;
        progress(`\nBridge to L2 confirmed:`);
        console.log(JSON.stringify(bridged, null, 2));

        // ── 9. Final assertions ───────────────────────────────────────────
        progress('\n=== STEP 9: Assertions ===');

        expect(rec.reservationId).toBe(reservationId);
        expect(typeof rec.originTxhash).toBe('string');
        expect(typeof rec.targetTxhash).toBe('string');
        expect(typeof rec.liteforgeTxhash).toBe('string');
        expect(['SETTLED', '4', 4]).toContain(rec.state);

        progress('\n✓ All assertions passed. Full LTC → Sepolia → L2 flow completed!');
        progress(`  Reservation ID:    ${reservationId}`);
        progress(`  LTC tx:            ${rec.originTxhash}`);
        progress(`  Settlement tx:     ${rec.targetTxhash}`);
        progress(`  Liteforge bridge:  ${rec.liteforgeTxhash}`);
